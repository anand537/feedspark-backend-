...
class MentorInput(afg_db.Model):
    ...
    evaluator_id = afg_db.Column(afg_db.UUID(as_uuid=True), afg_db.ForeignKey('users.id', ondelete='SET NULL'))
...
class PerformanceData(afg_db.Model):
    ...
    id = afg_db.Column(afg_db.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    mentor_input_id = afg_db.Column(afg_db.UUID(as_uuid=True), afg_db.ForeignKey('mentor_inputs.id', ondelete='CASCADE'))
    criterion_id = afg_db.Column(afg_db.UUID(as_uuid=True), afg_db.ForeignKey('criteria.id', ondelete='CASCADE'))
...
class Feedback(afg_db.Model):
    ...
    generated_at = afg_db.Column(afg_db.DateTime, default=datetime.utcnow)

class FeedbackVersion(afg_db.Model):
    __tablename__ = 'feedback_versions'

    id = afg_db.Column(afg_db.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    feedback_id = afg_db.Column(afg_db.UUID(as_uuid=True), afg_db.ForeignKey('feedbacks.id', ondelete='CASCADE'))
    feedback_text = afg_db.Column(afg_db.Text)
